...
expand_environ_var(char *arg)
{
	...
	if (arg[1] == '?') {
		sprintf(arg, "%i", status);  // write in arg the content of status
		return arg;
	}

	char *env_var;
	if ((env_var = getenv(&arg[1]))) {
		if (strlen(env_var) > strlen(arg)) {
			arg = realloc(arg, strlen(env_var) * sizeof(env_var));
...
parse_line(char *buf)
{
	...
	// para implementar pipes multiples
